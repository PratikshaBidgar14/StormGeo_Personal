...
define(['N/record', 'N/search', 'N/log'], function (record, search, log) {
    ...
    function onRequest(context) {
        try {
            ...
            log.debug('Received Parameters', { usageId, sfUphInternalId });
            ...
            if (usageStatus && usageStatus !== 'ACTIVE') {
                ...
                const usageCustomRecordInternalId = getCustomRecordInternalId(sfUphInternalId);
                ...
                if (usageCustomRecordInternalId) {
                    ...
                    log.audit('No Custom Record Found', sfUphInternalId);
                }
            }
    ...
    function getCustomRecordInternalId(sfUphInternalId) {
        const customRecordSearch = search.create({
            type: "customrecord_usage_status_record",
            filters: [
                ["custrecord_sf_uph_internal_id", "is", sfUphInternalId]
            ],
            columns: [
    ...
    function updateCustomRecordStatus(recordId, status) {
        record.submitFields({
            type: 'customrecord_usage_status_record',
            id: recordId,
            values: { custrecord_usage_status: status },
            options: {
